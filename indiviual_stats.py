...
def assist_stats(df):
    ...
    ast = ast.drop([ 'points','shots_made', 'FT_made', 'FT_missed', 'off_rebound','total_rebound', 'shots_missed', 'total_blocks', 'winner'], axis =1)
    return ast
...
def blk_stats(df):
    ...
    blk = blk.drop(['points', 'shots_made',	'FT_made',	'FT_missed', 'off_rebound',	'total_rebound','shots_missed', 'assist_count', 'winner'], axis =1)
    return blk
...
def player_names(df):
    ...
    plr_name = plr_name.drop(['assist_count', 'total_blocks', 'winner'], axis = 1)
    return plr_name
...
def merge_all(names,blocks,assists, df):
    ...
    result_2 = result_2[['player',	'shots_made', 'shots_missed', 'FT_made', 'FT_missed', 'assist_count', 'off_rebound', 'total_rebound', 'total_blocks']]
